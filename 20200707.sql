...
3. 모든 프랜차이즈를 한번만 읽고서 처리하는 방법
 ...
 NVL(SUM(DECODE(gb, '롯데리아', 1)), 1) score
 FROM fastfood
...
SELECT rank bg_rank, c.sido, c.sigungu, c.score bg_score,
        rank2 tax_rank, d.sido, d.sigungu, d.score tax_score 
 FROM
 ...
 GROUP BY sido, sigungu
 ORDER BY score DESC) a) c
 RIGHT OUTER JOIN 
 (SELECT ROWNUM rank2, a.*
 FROM
 (SELECT sido, sigungu, ROUND(sal/people, 2) score
 FROM tax
 ORDER BY score DESC) a) d ON(c.rank = d.rank2)
 ORDER BY tax_rank;
 
 
 도시발전 순위, 햄버거발전지수 시도, 햄버거 발전지수 시군구, 햄버거 발전지수
 근로소득순위, 근로소득 시도, 근로소득 시군구, 1인당 근로소득액
 
 
 CROSS JOIN : 테이블간 조인 조건을 기술하지 않는 형태로 두 테이블간
 행간 모든 가능한 조합으로 조인이 되는 형태
 ...
 * SQL의 중간 단계에서 필요한 경우는 존재
 ...
 서브쿼리에서 메인쿼리의 컬럼 사용유무에 따른 분류
 1. 서브쿼리에서 메인쿼리의 컬럼 사용 : correlated subquery ==> 상호 연관 서브쿼리
                    ==> 서브쿼리 단독으로 실행하는 것이 불가능
 2. 서브쿼리에서 메이쿼리의 컬럼 미사용 : non correlated subquery ==> 비상호 연관 서브쿼리
 ...
 우리가 원하는것은 고정된 부서번호로 사원 정보를 조회하는 것이 아니라
 SMITH가 속한 부서를 통해 데이터를 조회 ==> SMITH가 속한 부서가 바뀌더라도
 쿼리를 수정하지 않도록 하는 것
 
 위에서 작성한 두개의 쿼리를 하나로 합칠수 있다.
 ==> SMITH의 부서번호가 변경 되더라도 우리가 원한느 데이터 셋을
 ...
 * 제약사항 : 반드시 서브쿼리가 하나의 행, 하나의 컬럼을 반환 해야된다.
 ...
 IN-LINE VIEW : 그동안 많이 사용 FROM절에서 어떠한 서브쿼리를 괄호로 감싼.
