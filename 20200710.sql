...
방금 입력한 9999번 사원번호를 갖는 사원의 deptno와, job 컬럼의 값을
...
대량의 데이터를 지울때는 로그기록도 부하가 되기 때문에 
개발환경에서는 테이블의 모든 데이터를 지우는 경우에 한해서
TRUNCATE TABLE 테이블명; 명령을 통행
...
선행 트랜잭션 안에서 항상 동일한 데이터가 조회 되도록 보장하는 레벨
...
99번 부서의 부서명, loc 정보를 유지하고싶음
다른 트랜잭션에서 수정하지 못하도록 막고 싶음

후행 트랜잭션에서 업데이트, 커밋했을 경우 ==> 값이 바뀜

==트랜잭션 종료==

==트랜잭션 시작==
선행 트랜잭션에서 FOR UPDATE로 조회시 후행 트랜잭션에서 수정 불가능
(후행 트랜잭션에서 신규데이터를 입력하는 경우는 가능 - Phantom Read)
...
PHANTOM READ : LV2에서는 테이블에 존재하는 데이터에 대해 후행 트랜잭션에서 작업하지
못하도록 막을수는 있지만 후행트랜잭션에서 신규로 입력하는 데이터는 막을수 없다 
즉 선행 트랜잭션에서 처음 읽은 데이터화 후행트랜잭션에서 신규입력후 커밋한 이후에 
조회한 데이터가 불일치 할수 있다(없던 데이터가 갑자기 생성되는 현상)


LV3 : SERIALIZABLE READ
후행 트랜잭션이 데이터를 입력, 수정, 삭제 하더라도 선행트랜잭션에서는
...
DML(Data Manipulation Language) : 데이터를 다루는 SQL, 데이터 조작어
...
1. NOT NULL - CHECK 제약에 속함, 사용을 많이해서 별도로 키워드로 지정
...
4. FOREIGN KEY
    ...
    emp테이블과 dept테이블은 deptno 컬럼으로 연결 되어있음
...
5. CHECK 제약 조건
    ...
    ex) 어떤 테이블에 성별 컬럼이 존재하면
        ...
        C 성별을 입력하면?? 시스템 요구사항을 정의할 때 정의하지 않은 값이기 때문에 추후 문제가 될 수도 있다.
...
PRIMARY KEY 제약조건 확인 - UNIQUE + NOT NULL
...
컬럼명 컬럼타입 CONSTRAINT 제약조건명 제약조건타입(PRIMARY KEY)
